...
def read_data(filename):
    ...
    with zipfile.ZipFile(filename) as f:
        ...
        符分隔'''
    return data
words = read_data("D:/Downloads/text8.zip")
...
#5.生成Word2Vec的训练样本，使用skip-gram模式 (8,2,1)一个batch8个数据，每个目标词预测周围一共2个词，8条数据共包括了前个目标词
data_index = 0
...
def generate_batch(batch_size,num_skips,skip_window):
    ...
    :param num_skips: 每个单词生成的样本数量，不能超过skip_window的两倍，batch_size是num_skips的整数倍
    ...
    for i in range(batch_size // num_skips):
        ...
        #print(batch,labels)
        buffer.append(data[data_index])  # 'deque(3081,12,6)'更新buffer
        data_index = (data_index + 1) % len(data)
    return batch,labels
...
with graph.as_default():
    ...
    with tf.device("/gpu:0"):
        ...
        #使用tf.nn.embedding_lookup()函数查找train_inputs对应的向量embed，输入向量，独热向量或多热向量
        embed = tf.nn.embedding_lookup(embeddings,train_inputs)
        #使用截断正态函数初始化权重,偏重初始化为0
